...
public class AppSize extends ProduceableSubject<AppSizeInfo> implements Install<AppSizeConfig> {
    ...
    public synchronized boolean install(AppSizeConfig config) {
        ...
        mInstalled = true;
        mConfig = config;
        disposable = ThreadUtil.computationScheduler().scheduleDirect(new Runnable() {
            @Override
            public void run() {
                AppSizeUtil.getAppSize(GodEye.instance().getApplication(), new AppSizeUtil.OnGetSizeListener() {
                    @Override
