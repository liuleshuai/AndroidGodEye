...
class ImageCanaryInternal {
    ...
    void start(Application application, final ImageCanary imageCanaryEngine) {
        final Handler handler = ThreadUtil.createIfNotExistHandler(IMAGE_CANARY_HANDLER);
        callbacks = new SimpleActivityLifecycleCallbacks() {
            ...
            public void onActivityResumed(Activity activity) {
                ...
                final Runnable callback = inspectInner(new WeakReference<>(activity), imageCanaryEngine, mImageIssues);
    ...
    @VisibleForTesting
    Runnable inspectInner(final WeakReference<Activity> activity, final ImageCanary imageCanaryEngine, final Set<ImageIssue> imageIssues) {
        return new Runnable() {
            @Override
            public void run() {
                try {
                    ...
                    if (p != null) {
                        ...
                        ImageCanaryInternal.this.recursiveLoopChildren(p, parent, imageCanaryEngine, imageIssues);
                    }
                } catch (Throwable e) {
                    L.e(e);
                }
    ...
    private void recursiveLoopChildren(final Activity activity, ViewGroup parent, final ImageCanary imageCanaryEngine, final Set<ImageIssue> imageIssues) {
        ViewUtil.getChildren(parent, new ViewUtil.ViewFilter() {
            @Override
            public boolean isExclude(View view) {
                return false;
            }
        }, new ViewUtil.ViewProcess() {
            @Override
